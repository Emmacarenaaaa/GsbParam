<div class="detail-container">
    <?php if (isset($_SESSION['message_produit'])): ?>
        <div class="alert alert-success alert-dismissible fade show" role="alert" style="width: 100%; grid-column: 1 / -1; margin-bottom: 20px;">
    ...
    <div class="detail-right">
        <div class="detail-body">
            ...
            <div class="detail-contenance">
                ...
                <select class="select-box">
                    <option><?= htmlspecialchars($unProduit['contenanceProd'] ?? '') . ' ' . htmlspecialchars($unProduit['uniteProd'] ?? '') ?></option>
                </select>
            </div>
            ...
            <div class="detail-price-stock">
                ...
                <?php if ($unProduit['stockProd'] > 0): ?>
                    <span class="detail-stock" style="color: green;">- En Stock <span class="stock-count">(plus que <?= $unProduit['stockProd'] ?>)</span></span>
                <?php else: ?>
            ...
            <div class="detail-actions">
                ...
                <?php else: ?>
                    <button class="btn-ajouter" disabled style="background-color: #ccc; cursor: not-allowed; border: none; color: white; display: inline-block; text-align: center;">Ajouter au panier</button>
                <?php endif; ?>
            </div>
        </div>
        <div class="detail-footer">
            ...
            <?php if (isset($_SESSION['idHab']) && $_SESSION['idHab'] == 1): ?>
                <a href="index.php?uc=administration&action=afficherModifierProduit&idProd=<?= $unProduit['idProd'] ?>" class="btn-retour" style="background-color: #ffc107; color: black; margin-left: 10px;">Modifier</a>
            <?php endif; ?>
        </div>
    </div>
</div>
    ...
    <div class="card shadow-sm border-0 rounded-4 mb-5">
        <div class="card-body p-4 bg-light rounded-4">
            ...
            <?php if (!isset($_SESSION['idUser'])): ?>
                <p class="text-muted mb-0">Veuillez vous <a href="index.php?uc=connexion">connecter</a> pour laisser un avis sur ce produit.</p>
            <?php elseif ($aDejaAvis): ?>
                <p class="text-success fw-bold mb-0"><i class="bi bi-check-circle-fill me-2"></i>Vous avez déjà donné votre avis sur ce produit.</p>
            <?php else: ?>
                <form action="index.php?uc=voirProduits&action=ajouterAvis" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex" id="star-rating">
                            <i class="bi bi-star fs-4 text-warning me-1 star-btn" data-value="1" style="cursor: pointer;"></i>
                            <i class="bi bi-star fs-4 text-warning me-1 star-btn" data-value="2" style="cursor: pointer;"></i>
                            <i class="bi bi-star fs-4 text-warning me-1 star-btn" data-value="3" style="cursor: pointer;"></i>
                            <i class="bi bi-star fs-4 text-warning me-1 star-btn" data-value="4" style="cursor: pointer;"></i>
                            <i class="bi bi-star fs-4 text-warning me-1 star-btn" data-value="5" style="cursor: pointer;"></i>
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="3" placeholder="Partagez votre expérience avec ce produit..."></textarea>
                    </div>
                ...
                <div class="card shadow-sm border-0 rounded-4 mb-3">
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <h6 class="fw-bold mb-0"><?= htmlspecialchars($unAvis->prenomUser . ' ' . substr($unAvis->nomUser, 0, 1) . '.') ?></h6>
                            <span class="badge <?= $unAvis->note >= 4 ? 'bg-success' : ($unAvis->note == 3 ? 'bg-warning text-dark' : 'bg-danger') ?>">
